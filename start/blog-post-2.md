...
Hi guys! Welcome back to my Azure Function series. I hope u are ready to play, develop and (more important) learn with me. I'm really excited to be back on track 😎.  
🔴IMPORTANT❗🔴: this is the second post of a series so, if u missed the previous one, I strongly suggest u to check it out at this [link](https://dev.to/ivan_pesenti/playing-with-azure-functions-mission-1-55b8).  
...
If u have completed the MISSION 1 (congrats again 🍻) u now have the following weapons in your bag:
...
The first task is to tie our CRONTAB expression (which is responsible for defining **when** our function will run) to a key in the local.settings.json. This is done in order to be able to change it without redeploying our function and to make our function smarter. If u keep this expression within your source code (such as in TimerTriggerFunc.cs) when u want to change it you must do a full publish of your app. This is a pretty annoying stuff for a trivial change 🥱. So let's do this.  
...
```  
After that u have to add the key "TimerCron "in your local.settings.json, in your "Values" object:
...
⚠️WARNING⚠️: during class creation (especially if u are using VSCode like me) pay attention to the namespace that should reflect the position of the class within the proj. If the namespace generated **is different** please change it accordingly.  
...
### Services
Now it's e time to focus about the services section. This section will hold the core logic of our application. So, create a folder called "services" in the "azure-function-managers" proj. In this folder create a class called "GreetingsService.cs" with the following code:  
...
This will be our concrete implementation of the greetings functionality. What we need to do in order to use it through _**dependency injection**_ is to create an interface for it and make this class implement it. But wait! Instead of manually creating a file for the interface, populating it and changing the concrete class to implement it we're going to use the VSCode auto-completion feature to achieve it.  
Okay, let's do the trick 🛹.  
Open the GreetingsService class and put the cursor on its name. Then u must press the keyboard shortcut: "Ctrl" + "." and a little context menu will appear:  
<p align="center">
...
Make sure to select "Extract interface..." and after that the interface "IGreetingsService" will appear above your concrete class. Also note that the **concrete implementation** "GreetingsService.cs" now _implement_ it as u can see from this line:
```csharp
...
After this two touches of magic (every time I use these shortcuts I feel like a little wizard 🧙🏻‍♂️) we can go ahead and carry out other tasks.
...
🔴IMPORTANT❗🔴: be sure to put this section in the right place otherwise it will result in a compile time error. Make sure that you paste this code outside of the others "ItemGroup" sections.

## Creation of the Startup class

One of the most important thing we need to do is to build a class where we can resolve our dependencies at runtime (through the use of an **IOC container**) and provide the necessary configurations for our function. This place is called "Startup.cs".  
...
Nowadays it's fundamental to keep your confidential data safe 🔐. To achieve this we brought the .Net User Secrets into the scene. To start using them u have to install a Nuget package. If u are not already there goes to "azure-function" proj through the CLI and issue this statement:  
```shell
dotnet add package Microsoft.Extensions.Configuration.UserSecrets --version 3.1.16
```
🤑*BONUS*🤑: in order to manage your User Secrets in a similar way to the one provided by Visual Studio, u must install [this](https://marketplace.visualstudio.com/items?itemName=adrianwilczynski.user-secrets) VSCode extension.  
...
```  
Our Azure Function is not going to use this feature automatically. We must tell to take it into consideration when defining the settings for the function. So open up the "Startup.cs" again and add the following method below the Configure one:  
```csharp  
...
```  
With this piece of code we're telling how to configure our function. When u type this code the compiler start to complain so u have to add these using statements at the top of the "Startup.cs":  
```csharp  
...
Through this mission u learned how to develop a more complex Azure Function by taking advantage of DI, User Secrets, other class library projs and so on. U make your Azure Function more reliable, customizable, reusable and even more performant. Now u are prepared to accomplish your final mission 🤴🏻.  
